...
if (isset($_POST['user_id']) && isset($_POST['old_password']) && isset($_POST['new_password']) && isset($_POST['confirm_password'])) {
    ...
    $sql = "SELECT * FROM persident WHERE persident_id = :user_id";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
    $stmt->execute();
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($row) {
        if ($row['persident_password'] == md5($old_password)) {
            $password_md5 = md5($new_password);
            $sql = "UPDATE persident SET persident_password = :password_md5 WHERE persident_id = :user_id"; // corrected query
            $stmt = $pdo->prepare($sql);
