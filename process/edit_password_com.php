...
if (isset($_POST['user_id']) && isset($_POST['old_password']) && isset($_POST['new_password']) && isset($_POST['confirm_password'])) {
    ...
    $sql = "SELECT * FROM committee WHERE committee_id = :user_id";
    $stmt = $pdo->prepare($sql);
    $stmt->bindParam(':user_id', $user_id, PDO::PARAM_INT);
    $stmt->execute();
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if ($row) {
        if ($row['committee_password'] == md5($old_password)) {
            $password_md5 = md5($new_password);
            $sql = "UPDATE committee SET committee_password = :password_md5 WHERE committee_id = :user_id";
            $stmt = $pdo->prepare($sql);
