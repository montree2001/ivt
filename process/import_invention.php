...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['excelFile']) && $_FILES['excelFile']['error'] === UPLOAD_ERR_OK) {
        ...
        if ($sheet->getCellByColumnAndRow(1, 1)->getValue() != "รหัส" || $sheet->getCellByColumnAndRow(2, 1)->getValue() != "ชื่อผลงาน" || $sheet->getCellByColumnAndRow(3, 1)->getValue() != "สถานศึกษา" || $sheet->getCellByColumnAndRow(4, 1)->getValue() != "จังหวัด") {
            ...
            $sql = "SELECT invention_no FROM invention WHERE type_id = :type_id";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':type_id', $type_id, PDO::PARAM_INT);
            $stmt->execute();
            $invention_no = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $invention_no = array_column($invention_no, 'invention_no');


            for ($row = 2; $row <= $highestRow; ++$row) {
                $code = $sheet->getCellByColumnAndRow(1, $row)->getValue();
                if (in_array($code, $invention_no)) {
                    $_SESSION["alert_type"] = "error";
                    $_SESSION["alert_title"] = "เกิดข้อผิดพลาด";
            ...
            $sql = "INSERT INTO invention (invention_no,invention_name,invention_educational,type_id,invention_province)
            ...
            $_SESSION["alert_type"] = "error";
            $_SESSION["alert_title"] = "เกิดข้อผิดพลาด";
            $_SESSION["alert_message"] = "ไม่สามารถนำเข้าข้อมูลได้ กรุณาตรวจสอบไฟล์อีกครั้ง!";
            header("location: ../admin/invention_list.php?type_id=$type_id");
            exit();
        }
        }
